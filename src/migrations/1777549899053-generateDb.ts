...
export class GenerateDb1777549899053 implements MigrationInterface {
  name = 'GenerateDb1777549899053';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE TYPE "public"."order_status_enum" AS ENUM('created', 'proceed', 'payed', 'updated', 'in_progress', 'closed', 'failed')`,
    );
    await queryRunner.query(
    ...
    await queryRunner.query(
      `CREATE UNIQUE INDEX IF NOT EXISTS "Index_order_id_Unique" ON "processed_message" ("order_id") `,
    );
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "Index_created_at_order_status" ON "order" ("created_at", "order_status") `,
    );
  }

  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."Index_created_at_order_status"`,
    );
    await queryRunner.query(
      `DROP INDEX IF EXISTS "public"."Index_order_id_Unique"`,
    );
    await queryRunner.query(
      `CREATE TYPE "public"."order_status_enum_old" AS ENUM('created', 'proceed', 'updated', 'in_progress', 'closed', 'failed')`,
    );
    await queryRunner.query(
      `ALTER TABLE "order" ALTER COLUMN "order_status" DROP DEFAULT`,
    );
